  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm">
      <div className="space-y-3 p-3 md:hidden">
        {rows.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">No quotations found</p>
        ) : (
          rows.map((row) => {
            ...
            return (
              <MobileRecordCard key={row.id} selected={isSelected}>
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <Link href={`/quotations/${row.id}`} className="font-semibold text-indigo-600 hover:underline">
                        {row.quotationNumber}
                      </Link>
                ...
                <div className="space-y-2">
                  <MobileRecordRow label="Date">{formatDate(row.quotationDate)}</MobileRecordRow>
                  <MobileRecordRow label="Valid Until">
                ...
                <MobileRecordActions>
                  ...
                  <Button variant="outline" size="sm" className="gap-1.5 rounded-lg" onClick={() => onDownload(row)}>
                  ...
                  >
                    ...
                    {convertingId === row.id ? "Converting..." : "To Invoice"}
                  </Button>
