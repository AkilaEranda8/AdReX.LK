...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-[252px] flex-col border-r transition-all duration-300 ease-out lg:translate-x-0",
      ...
      >
        ...
        <nav className="flex-1 space-y-4 overflow-y-auto px-3 py-4">
          {navGroups.map((group) => (
            <div key={group.label}>
              <p className={cn("mb-1.5 px-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground", collapsed && "lg:hidden")}>
                  {group.label}
                </p>
        ...
        <div className="shrink-0 border-t border-slate-200/80 p-3 dark:border-slate-800">
          <div className={cn("mb-3", collapsed && "lg:hidden")} suppressHydrationWarning>
            <p className="mb-2 px-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              Appearance
            </p>
            <ThemeToggle />
          </div>
          <div className={cn("mb-2 hidden justify-center", collapsed && "lg:flex")} suppressHydrationWarning>
            <ThemeToggle compact />
          </div>
